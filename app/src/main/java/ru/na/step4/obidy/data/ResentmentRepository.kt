...
class ResentmentRepository(
    ...
    /** Default inventory category «Люди», or first available category. */
    suspend fun defaultCategoryId(): Long? {
        ensureDefaultCategories()
        val preferred = DefaultCategories.names.first()
        return categoryDao.getByName(preferred)?.id
            ?: categoryDao.getAll().firstOrNull()?.id
    }
