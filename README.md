...
**Важно для агента / Cursor:** деплой сервера и релиз (`agent_release.py` — bump, changelog, commit, push, сборка APK) не запускать, пока пользователь явно не попросит.
...
2. Собрать APK с новой версией и выложить: `python tools/publish_apk.py` (или вместе с релизом через `agent_release.py`).
...
Штатное обновление приходит из самого приложения: при запуске показывается окно с новой версией, а в чате «Техподдержка» есть активная кнопка обновления. Инструкция ниже — запасной путь через `adb`, если обновиться из приложения не получается.
